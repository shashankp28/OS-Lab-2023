...
static struct buf *rahead(struct inode *rip, block_t baseblock, u64_t position, unsigned bytes_ahead);
static int rw_chunk(struct inode *rip, u64_t position, unsigned off,
					size_t chunk, unsigned left, int rw_flag, cp_grant_id_t gid, unsigned buf_off, unsigned int block_size, int *completed);

/*--------------------------------------------*/
/*LAB-10*/
static void remove_inode_entry(struct inode *rip);
/*--------------------------------------------*/
...
int fs_readwrite(void)
{
	...
	regular = (mode_word == I_REGULAR || mode_word == I_NAMED_PIPE || mode_word == I_IMM);
	...
	while (nrbytes > 0)
	{
		...
		if (rw_flag == READING)
		{
			...
			if (chunk > (unsigned int)bytes_left)
				chunk = bytes_left;
		}
