...
int do_slink(void)
{
    ...
    if (vname1_length <= 1)
        return (ENOENT);
    if (vname1_length >= _POSIX_SYMLINK_MAX)
        return (ENAMETOOLONG);
...
struct fproc *rfp;
{
    ...
    if ((vp = eat_path(&resolve, rfp)) == NULL)
        return (err_code);
    ...
    if (r > 0)
        link_path[r] = '\0'; /* Terminate string when succesful */
